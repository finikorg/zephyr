...
static int rndis_send(struct net_pkt *pkt)
{
	...
	p = rndis_hdr_add(buf, net_pkt_get_len(pkt) + net_pkt_ll_reserve(pkt));

	if (!pkt->frags) {
		memcpy(p, net_pkt_ll(pkt), net_pkt_ll_reserve(pkt));
		p += net_pkt_ll_reserve(pkt);
	} else {
		struct net_buf *frag;

		memcpy(p, net_pkt_ll(pkt),
		       net_pkt_ll_reserve(pkt) + pkt->frags->len);
		p += net_pkt_ll_reserve(pkt) + pkt->frags->len;

		for (frag = pkt->frags->frags; frag; frag = frag->frags) {
			memcpy(p, frag->data, frag->len);
			p += frag->len;
		}
	}

	SYS_LOG_DBG("buf_len %u pktlen + ll %u", p - buf,
		    net_pkt_get_len(pkt) + net_pkt_ll_reserve(pkt));

	/* Split to CONFIG_RNDIS_BULK_EP_MPS */
	{
		int len = p - buf;
		int ret;
		...
		while (len) {
			...
			if (len < CONFIG_RNDIS_BULK_EP_MPS) {
				ret = try_write(CONFIG_RNDIS_IN_EP_ADDR, p,
						len);
				if (ret) {
